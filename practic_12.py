...
b = int(input('Коэффициент при x в sin-функции: '))  # Коэффициент при x в sin-функции
...
y = a*np.sin(b*x + c) - d*np.cos(g*x**2 + f*x + h)

# Построение графика
plt.plot(x, y, label='y=asin(bx+c)-bcos(gx^2+fx+h)')  # Линия графика
plt.axhline(0, color='red', linestyle='--', label='y=0')  # Ось x
plt.axvline(0, color='red', linestyle='--', label='x=0')  # Ось y
plt.xlabel('x')
plt.ylabel('y')
plt.title('График функции y=asin(bx+c)-dcos(gx^2+fx+h)')
plt.legend()  # Отображение легенды
plt.grid(True)  # Сетка

# Изменение цвета графика
plt.plot(x, y, color='blue', label=f'y=asin(bx+c)-dcos(gx^2+fx+h)')  # Линия графика (синий цвет)
